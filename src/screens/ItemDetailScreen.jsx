...
const ItemDetailScreen = () => {
  ...
  const handleMyItem = async () => {
    if (item.ownerId === user?.uid) {
      Alert.alert('Bu zaten senin ilanın!');
      return;
    }
  ...
  const handleContact = () => {
    Alert.alert(
      `${item.ownerName} ile İletişim`,
      `Bölüm: ${item.ownerDepartment}\nÖğrenci No: ${item.ownerStudentId || 'Belirtilmemiş'}`,
      [{ text: 'Tamam' }]
    );
  };
...
const styles = StyleSheet.create({
  ...
  infoValue: { fontSize: 14, fontWeight: 'bold', color: colors.onSurface, marginTop: 2 },
  ownerCard: {
  ...
  ownerName: { fontSize: 15, fontWeight: 'bold', color: colors.onSurface },
  ownerDept: { fontSize: 12, color: colors.textSecondary },
  bottomBar: {
  ...
  outlineBtnText: { color: colors.primary, fontSize: 14, fontWeight: 'bold' },
  primaryBtn: {
    flex: 1, height: 52, borderRadius: 12,
    backgroundColor: colors.secondary,
    alignItems: 'center', justifyContent: 'center',
